...
interface MenuItemProps extends MenuItemWrapperProps {
...
function MenuItem({ children, title, active, onClick }: MenuItemProps) {
  const handleClick = () => {
    onClick(title);
  };
...
export default function Menu({ activeItem, onItemClick }: MenuProps) {
  ...
  return (
    <MenuWrapper>
      <MenuItem title="Playlist" onClick={onClick} active={activeItem === 'Playlist'}>
        <svg
      ...
      <MenuItem title="Video" onClick={onClick} active={activeItem === 'Video'}>
